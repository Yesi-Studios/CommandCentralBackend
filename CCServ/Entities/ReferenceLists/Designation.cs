        ...
        public override void UpdateOrInsert(Newtonsoft.Json.Linq.JToken item, MessageToken token)
        {
            ...
            using (var transaction = session.BeginTransaction())
            {
                try
                {
                    ...
                    {
                        throw new CommandCentralException("The value, '{0}', already exists in the list.".FormatWith(designation.Value), ErrorTypes.Validation, System.Net.HttpStatusCode.BadRequest);
                        return;
                    }
        ...
        public override void Delete(Guid id, bool forceDelete, MessageToken token)
        {
            ...
            using (var transaction = session.BeginTransaction())
            {
                try
                {
                    ...
                    if (designation == null)
                    {
                        throw new CommandCentralException("That designation Id was not valid.", ErrorTypes.Validation, System.Net.HttpStatusCode.BadRequest);
                        return;
                    }
                    ...
                    if (persons.Any())
                    {
                        ...
                        else
                        {
                            ...
                            throw new CommandCentralException("We were unable to delete the designation, {0}, because it is referenced on {1} profile(s).".FormatS(designation, persons.Count), ErrorTypes.Validation, System.Net.HttpStatusCode.BadRequest);
                            return;
                        }
                    }
                    else
