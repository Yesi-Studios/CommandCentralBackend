...
namespace CommandDB_Plugin
{
    ...
    public static class AccountServices
    {
        ...
        /// WARNING!  THIS IS A CLIENT METHOD.  AUTHENTICATION, AUTHORIZATION AND VALIDATION MUST BE HANDLED PRIOR TO DB INTERACTION.
        /// <para />
        /// Logs out the user by invalidating the session in both the cache and by updating it in the database.
        /// <para />
        /// Options: 
        /// <para />
        ...
        /// <param name="token"></param>
        /// <returns></returns>
        public static async Task<MessageTokens.MessageToken> Logout(MessageTokens.MessageToken token)
        {
            try
            {
                //If the call got to here, then we know we have an authenticated session.

                await UnifiedServiceFramework.Authentication.Sessions.InvalidateSession(token.Session.ID);
        ...
        /// <para />
        /// Finally, we return the session id to be used as the authentication token for further requests.
        /// <para />
        /// Options: 
        /// <para />
        ...
        /// <param name="token"></param>
        /// <returns></returns>
        public static async Task<MessageTokens.MessageToken> Login(MessageTokens.MessageToken token)
        {
            try
            {
                //Check to make sure that we have all of the required arguments.
                if (!token.Args.ContainsKey("username"))
                    throw new ServiceException("You must send a username!", ErrorTypes.Validation);
                string username = token.Args["username"] as string;
                if (!ValidationMethods.IsValidUsername(username))
                    throw new ServiceException("That username was not in a valid username format.", ErrorTypes.Validation);

                if (!token.Args.ContainsKey("password"))
                    throw new ServiceException("You must send a password!", ErrorTypes.Validation);
                string password = token.Args["password"] as string;
                if (!ValidationMethods.IsValidPassword(password))
                    throw new ServiceException("That password could not be the correct password because it was not in the right format.", ErrorTypes.Validation);

                string personID, passwordHash = "";
                ...
                using (MySqlConnection connection = new MySqlConnection(Properties.ConnectionString))
                {
                    ...
                    using (MySqlCommand command = new MySqlCommand("", connection))
                    {
                        command.CommandText = "SELECT `ID`,`SecurePassword` FROM `persons_accounts` WHERE `Username` = @Username";
                        ...
                        using (MySqlDataReader reader = (MySqlDataReader)await command.ExecuteReaderAsync())
                        {
                            if (reader.HasRows)
                            {
                                ...
                                if (table.Rows.Count > 1)
                                    throw new Exception(string.Format("While conducting a login for the username '{0}', more than one profile was loaded for that username!", username));

                                personID = table.Rows[0]["ID"] as string;
                                passwordHash = table.Rows[0]["SecurePassword"] as string;
                            }
                            else
                            {
                                throw new ServiceException("Either the username or the password was incorrect.", ErrorTypes.Validation);
                            }
                        }
                ...
                var person = new
                {
                    ...
                    EmailAddress = (await EmailAddresses.DBLoadAll(personID)).FirstOrDefault(x => x.IsPreferred || x.IsContactable || x.IsDODEmailAddress)
                };

                //Now that we know we got a single profile, let's check the password.
                if (!PasswordHash.ValidatePassword(password, person.PasswordHash)) //THe password is not correct
                {

                    if (person.EmailAddress == null)
                    {
                        throw new Exception(string.Format("Login failed to the person's account whose ID is '{0}'; however, we could find no email to send this person a warning.", person.PersonID));
                    }


                    await EmailHelper.SendFailedAccountLoginEmail(person.EmailAddress.Address, person.PersonID);

                    throw new ServiceException("Either the username or the password was incorrect!", ErrorTypes.Validation);
                }

                //At this point, we know that the client is allowed to do a login.
                //First, we're going to load the client's permissions
                List<string> permissionIDs = (await CustomAuthorization.CustomPermissions.GetPermissionGroupsForUser(person.PersonID)).Select(x => x.ID).ToList();

                //Then we're going to create a new session!  Keep the auth token to give to the client.
                string authenticationToken = await UnifiedServiceFramework.Authentication.Sessions.CreateNewSession(person.PersonID, permissionIDs);

                //Right before we return to the client, let's insert this account history event.
                await new AccountHistoryEvents.AccountHistoryEvent()
                {
                ...
                //And then return the token!  let's give the client both their ID and the auth token.
                token.Result = new { PersonID = personID, AuthenticationToken = authenticationToken };
